# CJS QA Automation Framework(s) - Dockerfile
...
# Optimized: Added cleanup of Maven cache after build to reduce image size
# Clear cached failures and force update to ensure fresh downloads
# Use 'mvn' directly since the base image already has Maven 3.9.9 installed
RUN rm -rf ~/.m2/repository/org/apache/maven/plugins/maven-compiler-plugin || true \
    && mvn -ntp -U clean package -DskipTests \
    && rm -rf ~/.m2/repository/*/SNAPSHOT \
    && rm -rf ~/.m2/repository/.cache

# Stage 2: Runtime stage
# Optimized: Using JRE instead of JDK (saves ~100-150MB, tests are pre-compiled)
...
# Optimized: Added cleanup to reduce layer size
WORKDIR /app/cypress
