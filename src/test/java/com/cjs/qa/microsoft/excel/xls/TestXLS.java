...
public class TestXLS {
  ...
  private static final GuardedLogger LOG = new GuardedLogger(LogManager.getLogger(TestXLS.class));

  private static final String FILE_NAME = "existing" + IExtension.XLS;
  ...
  private void createFontsColumns(XLS excel, String sheetName) throws QAException {
    for (int column = 0; column < IExcel.CELL_STYLE_LIST.size(); column++) {
      final XLSCellStyles xlsCellStyles = XLSCellStyles.getStylesList().get(column);
  ...
  private void createFontsRows(XLS excel, String sheetName) throws QAException {
    for (int row = 0; row < IExcel.CELL_STYLE_LIST.size(); row++) {
      final XLSCellStyles xlsCellStyles = XLSCellStyles.getStylesList().get(row);
      final String format = xlsCellStyles.getStyleName();
  ...
  private void createFontsCells(XLS excel, String sheetName) throws QAException {
