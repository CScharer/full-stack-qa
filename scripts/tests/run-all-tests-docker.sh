...
run_test_suite "Robot Framework Tests" \
    "docker-compose run --rm -e BASE_URL=$BASE_URL -e ENVIRONMENT=$ENVIRONMENT -e SELENIUM_REMOTE_URL=$SELENIUM_REMOTE_URL tests bash -c './mvnw test -Dmaven.test.skip=false -Probot'"
