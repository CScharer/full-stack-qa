...
# We only need to verify/rebuild for Allure3 or if Allure2 created object format (which shouldn't happen)
if [ -f "$REPORT_DIR/history/history-trend.json" ] && command -v jq &> /dev/null; then
    ...
    # We should only rebuild if it has object format (which shouldn't happen for Allure2)
