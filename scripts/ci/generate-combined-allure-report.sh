...
# NOTE: For Allure2, history-trend.json is automatically generated by 'allure generate'
# Allure2 automatically merges previous history, so we should NOT rebuild unless absolutely necessary
# For Allure2, object format (aggregated statistics) might actually be valid for trend charts
if [ -f "$REPORT_DIR/history/history-trend.json" ] && command -v jq &> /dev/null; then
    echo ""
    echo "🔍 Verifying history-trend.json format..."
    
    # For Allure2, check entry count first - if we have multiple buildOrder entries, Allure2 did its job correctly
    ENTRY_COUNT=$(jq 'length' "$REPORT_DIR/history/history-trend.json" 2>/dev/null || echo "0")
    echo "   Found $ENTRY_COUNT buildOrder entry/entries in history-trend.json"
    ...
    # NOTE: For Allure2, 'allure generate' automatically creates history-trend.json
    # For Allure2, if we have multiple entries, we should preserve Allure2's format
    # Only rebuild if we have object format AND only 1 entry (indicating previous history was lost)
    HAS_OBJECT_DATA=$(jq '[.[] | select(.data | type == "object")] | length' "$REPORT_DIR/history/history-trend.json" 2>/dev/null || echo "0")
    
    # For Allure2: Check if Allure2 created object format (aggregated statistics)
    # Allure2 trend charts may actually NEED aggregated statistics format, not individual test items
    # Only rebuild if we have object format AND only 1 entry (indicating history was lost)
    # If we have multiple entries with object format, Allure2 is working correctly - preserve it
    if [ "$ALLURE_VERSION" = "2" ]; then
        if [ "$ENTRY_COUNT" -gt 1 ]; then
            echo "   ✅ Allure2 preserved $ENTRY_COUNT buildOrder entries - history is intact"
            if [ "$HAS_OBJECT_DATA" != "0" ] && [ "$HAS_OBJECT_DATA" != "null" ]; then
                echo "   ℹ️  Allure2 created object format (aggregated statistics) - this is valid for Allure2 trend charts"
                echo "   ✅ Preserving Allure2's aggregated statistics format (multiple entries indicate history is working)"
                HAS_OBJECT_DATA="0"  # Don't rebuild - Allure2's format is correct
            fi
        elif [ "$HAS_OBJECT_DATA" != "0" ] && [ "$HAS_OBJECT_DATA" != "null" ] && [ "$ENTRY_COUNT" -eq 1 ]; then
            # Only 1 entry with object format - previous history was lost, need to rebuild
            echo "   ⚠️  Allure2 created object format but only 1 entry - previous history may have been lost"
            echo "   🔧 Rebuilding to preserve any previous history and convert to array format"
        fi
    fi
    ...
    if [ "$HAS_OBJECT_DATA" != "0" ] && [ "$HAS_OBJECT_DATA" != "null" ]; then
        ...
        # Also preserve entries from RESULTS directory (downloaded previous history)
        # This ensures we don't lose historical buildOrder entries from previous runs
        # Preserve both array-format AND object-format entries (we'll convert object-format later)
        if [ -f "$RESULTS_DIR/history/history-trend.json" ]; then
            PREVIOUS_ENTRIES=$(jq '.' "$RESULTS_DIR/history/history-trend.json" 2>/dev/null || echo "[]")
            if [ "$PREVIOUS_ENTRIES" != "[]" ] && [ "$PREVIOUS_ENTRIES" != "null" ]; then
                PREVIOUS_COUNT=$(echo "$PREVIOUS_ENTRIES" | jq 'length' 2>/dev/null || echo "0")
                if [ "$PREVIOUS_COUNT" != "0" ] && [ "$PREVIOUS_COUNT" != "null" ] && [ "$PREVIOUS_COUNT" -gt 0 ]; then
                    # Extract array-format entries from previous history
                    PREVIOUS_ARRAY_ENTRIES=$(echo "$PREVIOUS_ENTRIES" | jq '[.[] | select(.data | type == "array")]' 2>/dev/null || echo "[]")
                    ARRAY_COUNT=$(echo "$PREVIOUS_ARRAY_ENTRIES" | jq 'length' 2>/dev/null || echo "0")
                    
                    if [ "$ARRAY_COUNT" != "0" ] && [ "$ARRAY_COUNT" != "null" ] && [ "$ARRAY_COUNT" -gt 0 ]; then
                        ...
                        echo "   ✅ Preserved $ARRAY_COUNT array-format entry/entries from previous history"
                    fi
                    
                    # Also check for object-format entries that we need to convert
                    PREVIOUS_OBJECT_COUNT=$(echo "$PREVIOUS_ENTRIES" | jq '[.[] | select(.data | type == "object")] | length' 2>/dev/null || echo "0")
                    if [ "$PREVIOUS_OBJECT_COUNT" != "0" ] && [ "$PREVIOUS_OBJECT_COUNT" != "null" ] && [ "$PREVIOUS_OBJECT_COUNT" -gt 0 ]; then
                        echo "   ℹ️  Found $PREVIOUS_OBJECT_COUNT object-format entry/entries in previous history (will be converted)"
                    fi
                fi
            fi
        fi
    ...
    # NOTE: For Allure2, widgets/history-trend.json is automatically generated by 'allure generate'
    # We should only copy if Allure2 didn't create it (which shouldn't happen)
    if [ -d "$REPORT_DIR/widgets" ]; then
        echo ""
        echo "📊 Ensuring widgets/history-trend.json exists for UI..."
        if [ ! -f "$REPORT_DIR/widgets/history-trend.json" ]; then
            # Only copy if widgets version doesn't exist (Allure2 should have created it)
            ...
            echo "   ✅ widgets/history-trend.json already exists (created by Allure2)"
        fi
    fi
fi
